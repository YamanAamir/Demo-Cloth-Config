...
const ZippedHoodie = ({ data, onUpdate, isAppReady, loadedTrigger, logos, backDesigns, onOpenInquiry, activeTab: externalTab, onTabChange, maxCharsText = 25, libDesignColor: libDesignColorProp, setLibDesignColor }) => {
  ...
  const sendAllCustomizations = React.useCallback(() => {
    console.log("📤 [ZippedHoodie] Sending all customizations to PlayCanvas on loaded event");
    lastSentRef.current = {};
    ...
    areas.forEach((area) => {
      ...
      postToActivePreview(`ZipperHoodie:${area}_opacity: ${opacity}`);
      ...
      getDiffuseBase64(flag, logoPre, logoCustom, text, (diffuseBase, logoOpacityBase) => {
        ...
        postToActivePreview(`ZipperHoodie:${area}_diffuse: ${diffuseBase}`);
        if (logoOpacityBase) postToActivePreview(`ZipperHoodie:${area}_opacity: ${logoOpacityBase}`);
    ...
    if (lastBackDesignPayloadRef.current) {
      ...
      if (color === 'black') {
        if (diffuseB64) postToActivePreview("ZipperHoodie:back_white_diffuse: " + diffuseB64);
        if (invertedB64) postToActivePreview("ZipperHoodie:back_white_opacity: " + invertedB64);
      } else if (color === 'white') {
    ...
    if (update.canvasBase64) {
      ...
      if (color === 'black' && opacityB64) {
        ...
        img.onload = () => {
          ...
          postToActivePreview("ZipperHoodie:back_white_diffuse: " + diffuseB64);
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          postToPreview(`zipper backDesign`);
                        }}
