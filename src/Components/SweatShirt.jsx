...
const SweatShirt = ({ data, onUpdate, isAppReady, loadedTrigger, logos, backDesigns, onOpenInquiry, activeTab: externalTab, onTabChange, maxCharsText = 25, libDesignColor: libDesignColorProp, setLibDesignColor }) => {
  ...
  useEffect(() => {
    ...
    postIfChanged("size", `SweatShirt:size:${selectedSize}`);
    ...
    if (update.canvasBase64) {
      ...
      if (color === 'black' && opacityB64) {
        ...
        img.onload = () => {
          ...
          postToActivePreview("SweatShirt:back_white_diffuse: " + cleanDiffuseB64);
        ...
        img.onload = () => {
          ...
          postToActivePreview("SweatShirt:back_black_diffuse: " + cleanDiffuseB64);
          postToActivePreview("SweatShirt:back_black_opacity: " + opacityB64);
        };
        img.src = opacityB64;
      }
    }
                ...
                <button
                  ...
                  style={{
                    ...
                    boxShadow: selectedColor === color.name
                      ? `0 0 0 2px white, 0 0 0 3px ${color.border}`
                      : "none",
                  }}
                >
                  {selectedColor === color.name && (
                    <div className="absolute inset-0 rounded-md border border-white pointer-events-none" />
                  )}
                </button>
              ))}
            </div>
